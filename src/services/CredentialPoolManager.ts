...
const UPLOAD_POOL_KEY = 'UPLOAD_CREDENTIAL_POOL'; // Upload-only pool for validation
...
export class CredentialPoolManager {
  ...
  /**
   * Add credential to upload-only pool during validation.
   */
  async addUploadCredential(credentialId: number) {
    await redis.lpush(UPLOAD_POOL_KEY, String(credentialId));
    console.log(`[PoolManager] Added credential ${credentialId} to upload pool.`);
  }

  /**
   * Remove credential from upload-only pool after validation.
   */
  async removeUploadCredential(credentialId: number) {
    await redis.lrem(UPLOAD_POOL_KEY, 0, String(credentialId));
  }
