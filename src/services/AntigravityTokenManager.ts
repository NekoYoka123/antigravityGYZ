...
export class AntigravityTokenManager {
    ...
    private async _doRefreshToken(tokenId: number): Promise<boolean> {
        ...
        try {
            ...
            if (token.fail_count > 0) {
                try {
                    await prisma.antigravityToken.update({ where: { id: tokenId }, data: { fail_count: 0 } });
                } catch { }
            }
            return true;
        } catch (error: any) {
            ...
            // 429/5xx: Temporary error, do NOT increase fail_count, do NOT mark dead.
            const isTemp = status === 429 || status === 503 || status === 502 || status === 500;
            if (isTemp) {
                console.warn(`[AntigravityTokenManager] Token #${tokenId} refresh temp fail (${status}). Retrying next time.`);
                return false;
            }

            if (status === 403) {
                // 2-Strike Rule
                if (token.fail_count >= 1) {
                    ...
                    if (withinGrace) {
                        ...
                        console.error(`[AntigravityTokenManager] Token #${tokenId} refresh 403 x2 (Strike 2). Marking DEAD.`);
                        await this.markAsDead(tokenId);
                    }
                } else {
                    console.warn(`[AntigravityTokenManager] Token #${tokenId} refresh 403 (Strike 1).`);
                    await prisma.antigravityToken.update({
                        where: { id: tokenId },
                ...
                // Unknown error (e.g. 400), maybe permanent? increment fail count safely.
                await prisma.antigravityToken.update({
                    where: { id: tokenId },
                    data: { fail_count: { increment: 1 } }
                });
