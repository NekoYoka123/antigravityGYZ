    ...
    static async generateStreamResponse(
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const tryStream = () => {
                ...
                streamResponse
                    .onStart(async ({ status, headers: respHeaders }) => {
                        statusCode = status;

                        // Retry Logic for Initial Handshake Failure
                        if ((status === 429 || status === 503 || status === 502 || status === 500) && attempt < MAX_RETRIES) {
                            ...
                            if (retryAfter) {
                                ...
                                if (!isNaN(secs) && secs > 0) waitMs = secs * 1000;
                            }
                            if (waitMs === 0) {
                                // Full Jitter
                                const base = 1000;
                                ...
                                if (waitMs < 500) waitMs = 500 + Math.floor(Math.random() * 500);
                            }

                            console.warn(`[AntigravityService] 流式请求握手失败 ${status}, 重试 ${attempt + 1}/${MAX_RETRIES}, 等待 ${waitMs}ms`);
                            attempt++;
                            await new Promise(r => setTimeout(r, waitMs));
                            tryStream(); // Recursive Retry
                            return;
                        }

                        if (status !== 200) {
                            // Will accumulate body and reject in onEnd
                        }
                    })
                    .onData((chunk) => {
                        if (hasRetried) return; // Ignore stale chunks from failed attempt (though fetchStream usually stops)
                        if (statusCode !== 200) {
                            errorBody += chunk;
                        } else {
                            processChunk(chunk);
                    ...
                    .onError((err) => {
                        ...
                        if (attempt < MAX_RETRIES) {
                            hasRetried = true;
                            const waitMs = 1000 * (attempt + 1); // Simple backoff for network
                            console.warn(`[AntigravityService] 流式请求网络错误 ${err.message}, 重试 ${attempt + 1}/${MAX_RETRIES}`);
                            attempt++;
                            setTimeout(tryStream, waitMs);
                            return;
                        }
                        reject(err);
                    });
        ...
        if (opts?.retry_on_429) {
            ...
            while (attempt < max) {
                ...
                if (waitMs === 0) {
                    ...
                    // base = 1000ms, cap = 8000ms
                    // sleep = min(cap, base * 2^attempt)
                    // jitter = random(0, sleep)
                    const base = 1000;
                    const cap = 10000;
                    const exp = Math.min(cap, base * Math.pow(2, attempt));
                    waitMs = Math.floor(Math.random() * exp);

                    // Ensure at least minimal wait
                    if (waitMs < 500) waitMs = 500 + Math.floor(Math.random() * 500);
                }

                console.warn(`[AntigravityService] 验证遇到 ${status}, 重试 ${attempt + 1}/${max}, 等待 ${waitMs}ms`);
                await new Promise(r => setTimeout(r, waitMs));
