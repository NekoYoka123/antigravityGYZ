...
export function isThinkingModel(modelName: string): boolean {
    if (modelName.includes('-image-')) return false;
...
export function getThinkingBudget(modelName: string): number {
    if (modelName.includes('-image-')) return 0;
    // 8k 版本
...
export function normalizeThinkingLevel(modelName: string, params: any = {}): string | null {
    if (!modelName.startsWith('gemini-3-')) return null;
    if (modelName.includes('-image-')) return null;

    const raw = params.thinking_level
        ...
        ?? params.generationConfig?.thinkingConfig?.thinking_level
        ?? params.generation_config?.thinking_config?.thinking_level
        ?? params.generation_config?.thinking_config?.thinkingLevel;
    if (typeof raw === 'string' && raw.trim() !== '') {
        return raw.trim().toLowerCase();
    }

    if (modelName === 'gemini-3-pro-low') return 'low';
    return null;
}

export function normalizeImageConfig(params: any = {}): { aspectRatio?: string; imageSize?: string } | null {
    const rawConfig = params.imageConfig
        ?? params.image_config
        ?? params.generationConfig?.imageConfig
        ?? params.generationConfig?.image_config
        ?? params.generation_config?.imageConfig
        ?? params.generation_config?.image_config;

    const base = rawConfig && typeof rawConfig === 'object' ? rawConfig : params;
    const aspectRatio = base?.aspectRatio ?? base?.aspect_ratio;
    const imageSize = base?.imageSize ?? base?.image_size;

    const imageConfig: { aspectRatio?: string; imageSize?: string } = {};
    if (aspectRatio) imageConfig.aspectRatio = String(aspectRatio);
    if (imageSize) imageConfig.imageSize = String(imageSize);
...
export function mapModelName(modelName: string): string {
    ...
    if (baseName === 'gemini-3-pro-image-preview') return 'gemini-3-pro-image';
    // gemini-3-pro-low 走 high 模型 + thinkingLevel=low
    if (baseName === 'gemini-3-pro-low') return 'gemini-3-pro-high';
...
export function generateAntigravityRequestBody(
    ...
    const thinkingLevel = normalizeThinkingLevel(modelName, params);
    const imageConfig = normalizeImageConfig(params);
