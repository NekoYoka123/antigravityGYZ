...
async function loadCliQuotaLimits(activeCredCount: number, activeV3CredCount: number): Promise<CliQuotaLimits> {
  const configSetting = await prisma.systemSetting.findUnique({ where: { key: 'SYSTEM_CONFIG' } });
  let totalQuota = 300;
  let limits: CliQuotaLimits = { flash: totalQuota, pro: totalQuota, v3: totalQuota, total: totalQuota };
  if (configSetting) {
    try {
      const conf = JSON.parse(configSetting.value);
      limits = calculateCliQuotaLimits(conf, activeCredCount, activeV3CredCount);
    } catch (e) {
...
export class GoogleAIController {
  ...
  static async generateContent(req: FastifyRequest, reply: FastifyReply) {
    ...
    if (!isAdminKey) {
      cliQuotaLimits = await loadCliQuotaLimits(activeCredCount, activeV3CredCount);
      if (user.today_used >= cliQuotaLimits.total) {
        return reply.code(402).send({ error: `每日配额已用完 (${user.today_used}/${cliQuotaLimits.total})` });
      }
      ...
      if (group !== 'other') {
        try {
          ...
          console.error('[GoogleAI] 模型配额检查失败:', e?.message || e);
        }
      }
    }
  ...
  static async streamGenerateContent(req: FastifyRequest, reply: FastifyReply) {
    ...
    if (!isAdminKey) {
      cliQuotaLimits = await loadCliQuotaLimits(activeCredCount, activeV3CredCount);
      if (user.today_used >= cliQuotaLimits.total) {
        reply.code(402).send({ error: `每日配额已用完 (${user.today_used}/${cliQuotaLimits.total})` });
        return;
      }
      const { group } = resolveCliUsageGroup(realModelName);
      if (group !== 'other') {
        try {
          ...
          const limit = cliQuotaLimits[group];
          if (Number.isFinite(limit) && used >= limit) {
            reply.code(402).send({ error: `模型 ${realModelName} 每日配额已用完 (${used}/${limit})` });
            return;
          }
        } catch (e: any) {
          console.error('[GoogleAI] 模型配额检查失败:', e?.message || e);
