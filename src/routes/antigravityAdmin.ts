...
async function verifyAdmin(req: FastifyRequest, reply: FastifyReply): Promise<boolean> {
    const authHeader = req.headers.authorization;
    // Allow token via query for SSE (EventSource cannot set headers)
...
export default async function antigravityAdminRoutes(app: FastifyInstance) {
    ...
    app.post('/config', async (req, reply) => {
        ...
        const config = {
            ...
            rate_limit_increment: typeof rate_limit_increment === 'number' ? rate_limit_increment : 0,
    ...
    app.get('/stats', async (req, reply) => {
        ...
        const setting = await prisma.systemSetting.findUnique({ where: { key: 'ANTIGRAVITY_CONFIG' } });
        let config = { claude_limit: 100, gemini3_limit: 200, claude_token_quota: 100000, gemini3_token_quota: 200000, rate_limit_increment: 0 };
        if (setting) {
    ...
    app.get('/tokens', async (req, reply) => {
        ...
        const filtered = pool === 'Pro'
            ? enriched.filter((x: any) => x.classification === 'Pro')
            : pool === 'Normal'
                ? enriched.filter((x: any) => x.classification === 'Normal')  // 精确匹配 Normal
                : enriched;
    ...
    app.get('/quotas/refresh', async (req, reply) => {
        ...
        const { antigravityTokenManager } = require('../services/AntigravityTokenManager');
        const limit = pLimit(120); // higher concurrency for admin-triggered refresh
        const results: any[] = [];
        let refreshed = 0;
        let pro = 0, normal = 0, unknown = 0, errors = 0;
        ...
        await Promise.all(tokens.map(t => limit(async () => {
            ...
            try {
                ...
                results.push({ id: t.id, status: 'ok', classification: cls || null });
            } catch (e: any) {
        ...
        return { success: true, refreshed, total: tokens.length, pro, normal, unknown, errors, duration_ms: Date.now() - startedAt, results };
    });

    // 刷新所有凭证额度缓存 (仅管理员) - SSE 流式输出
    app.get('/quotas/refresh-stream', async (req, reply) => {
        if (!await verifyAdmin(req, reply)) return;
        ...
        const tokens = await prisma.antigravityToken.findMany({
            ...
            select: { id: true, access_token: true, refresh_token: true, expires_in: true, timestamp: true, project_id: true, status: true, email: true }
        });

        const { antigravityQuotaCache } = require('../services/AntigravityQuotaCache');
        const { antigravityTokenManager } = require('../services/AntigravityTokenManager');
        const limit = pLimit(120); // Higher concurrency for faster refresh
        let completed = 0;
        ...
        // Send a normal message event so the frontend's onmessage handler receives it
    ...
    app.get('/pools/overview', async (req, reply) => {
        ...
        const { antigravityQuotaCache } = require('../services/AntigravityQuotaCache');
        let normal = 0, pro = 0, unknown = 0;
        const normalVals: number[] = [];
        const proVals: number[] = [];
        for (const t of tokens) {
            // 优先读内存缓存，若缺失尝试 Redis 快照
            let c = antigravityQuotaCache.get(t.id);
            if (!c) {
        ...
        return {
            counts: { normal, pro, unknown, active: tokens.filter(t => t.status === 'ACTIVE').length, cooling: tokens.filter(t => t.status === 'COOLING').length },
    ...
    app.get('/tokens/:id/quotas', async (req, reply) => {
        ...
        try {
            ...
            if (status === 403) {
                return reply.code(400).send({ error: '该凭证无权限查看额度', detail: message });
            }
    ...
    app.post('/health-check', async (req, reply) => {
        ...
        await Promise.all(allTokens.map(token => limit(async () => {
            results.checked++;
    ...
    // Antigravity 流式检活 (SSE + 高并发)
    app.get('/health-check/stream', async (req, reply) => {
        if (!await verifyAdmin(req, reply)) return;
        ...
        const limit = pLimit(50); // Concurrency 50 for speed

        try {
            if (type === 'ag_active') {
                // --- Active Check ---
                sendEvent({ type: 'log', message: '🚀 开始反重力活跃凭证检活 (并发数: 50)...' });
                const tokens = await prisma.antigravityToken.findMany({
                    where: { status: 'ACTIVE', is_enabled: true },
                    include: { owner: { select: { id: true, username: true, level: true } } }
                });

                sendEvent({ type: 'init', total: tokens.length });
                ...
                await Promise.all(tokens.map(token => limit(async () => {
                    try {
                        ...
                        const result = await AntigravityService.generateResponse(
                            ...
                            ANTIGRAVITY_MODELS[0] || 'gemini-3-pro-preview',
                            {}, undefined, testToken as any
                        );

                        if (!result.content && (!result.toolCalls || result.toolCalls.length === 0)) {
                            // Empty response is fine for liveness check usually
                        }

                        healthy++;
                        sendEvent({ type: 'log', message: `✅ #${token.id} 健康` });
                        ...
                        if (msg.includes('403')) {
                            ...
                            dead++;
                            sendEvent({ type: 'log', message: `❌ #${token.id} 403 权限不足 -> 失效` });
                        } else if (msg.includes('429')) {
                            const cooldownTime = new Date(Date.now() + 5 * 60 * 60 * 1000);
                            await prisma.antigravityToken.update({
                                where: { id: token.id },
                                data: { status: 'COOLING', cooling_until: cooldownTime }
                            });
                            cooled++;
                            sendEvent({ type: 'log', message: `⏳ #${token.id} 429 -> 冷却` });
                        } else {
                            // Transient error
                            sendEvent({ type: 'log', message: `⚠️ #${token.id} 异常: ${msg}` });
                        }
                    } finally {
                        processed++;
            ...
            } else if (type === 'ag_dead') {
                // --- Dead Check (Re-check dead tokens) ---
                sendEvent({ type: 'log', message: '💀 开始反重力失效凭证复检...' });
                const tokens = await prisma.antigravityToken.findMany({
                    where: { status: 'DEAD' },
                ...
                await Promise.all(tokens.map(token => limit(async () => {
                    try {
                        ...
                        const result = await AntigravityService.generateResponse(
                            [{ role: 'user', content: 'hi' }],
                            ANTIGRAVITY_MODELS[0] || 'gemini-3-pro-preview',
                            { max_tokens: 10 }, undefined, testToken as any
                        );
                        ...
                        if (result.content && result.content.length > 0) {
                            ...
                            await prisma.antigravityToken.update({
                                where: { id: token.id },
                                data: { status: 'ACTIVE', is_enabled: true }
                            });
                            healthy++;
                            sendEvent({ type: 'log', message: `🎉 #${token.id} 复活成功!` });
                        } else {
                            dead++;
                            sendEvent({ type: 'log', message: `❌ #${token.id} 仍无响应` });
                        }
                    } catch (e: any) {
                        const msg = e.message || '';
                        if (msg.includes('429')) {
                            const cooldownTime = new Date(Date.now() + 5 * 60 * 60 * 1000); // 5 hours cooling
                            // Revive but cool
                            await prisma.antigravityToken.update({
                                where: { id: token.id },
                                data: { status: 'COOLING', is_enabled: true, cooling_until: cooldownTime }
                            });
                            cooled++;
                            sendEvent({ type: 'log', message: `⏳ #${token.id} 复活并进入冷却 (429)` });
                        } else {
                            dead++;
                            sendEvent({ type: 'log', message: `❌ #${token.id} 仍失效: ${msg}` });
                        }
                    } finally {
                        processed++;
                        sendEvent({ type: 'progress', processed, healthy, dead, cooled });
            ...
            } else if (type === 'ag_enable_dead') {
                // --- One-click Enable Force ---
                sendEvent({ type: 'log', message: '⚡ 开始一键启用反重力失效凭证...' });
                const tokens = await prisma.antigravityToken.findMany({
                    where: { status: 'DEAD' },
                    select: { id: true, access_token: true, refresh_token: true, expires_in: true, timestamp: true, project_id: true, email: true }
                });
                    ...
                    try {
                        ...
                        try {
                            const result = await AntigravityService.generateResponse(
                                [{ role: 'user', content: 'hi' }],
                                ANTIGRAVITY_MODELS[0] || 'gemini-3-pro-preview',
                                { max_tokens: 5 }, undefined, testToken as any
                            );

                            if (result.content) {
                                await prisma.antigravityToken.update({ where: { id: token.id }, data: { status: 'ACTIVE', is_enabled: true } });
                                activated++;
                                sendEvent({ type: 'log', message: `🎉 #${token.id} 启用成功` });
                            } else {
                                failed++;
                                sendEvent({ type: 'log', message: `❌ #${token.id}启用失败: 无内容` });
                            }
                        } catch (e: any) {
                            if (e.message?.includes('429')) {
                                ...
                                activated++;
                                sendEvent({ type: 'log', message: `⏳ #${token.id} 启用并冷却 (429)` });
                            } else {
                                failed++;
                                sendEvent({ type: 'log', message: `❌ #${token.id} 启用失败: ${e.message}` });
                            }
                        }
                    } catch (e) {
                        failed++;
    ...
    // SSE 端点：监控后台自动刷新进度 (管理员)
    // 订阅 CronService 的自动刷新进度事件
    app.get('/quotas/refresh-monitor', async (req, reply) => {
        if (!await verifyAdmin(req, reply)) return;
        ...
        sendSSE({ type: 'connected', message: '已连接到自动刷新监控，等待下次刷新...' });

        // Subscribe to channel
        subscriber.subscribe(REFRESH_CHANNEL);
        subscriber.on('message', (channel: string, message: string) => {
            if (channel === REFRESH_CHANNEL) {
                try {
        ...
        req.raw.on('close', () => {
            subscriber.unsubscribe(REFRESH_CHANNEL);
