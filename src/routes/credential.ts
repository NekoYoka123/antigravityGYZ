...
export default async function credentialRoutes(fastify: FastifyInstance) {
   ...
   * @returns 禁用结果
   */
  fastify.post('/:id/disable', async (req, reply) => {
    const { id } = req.params as { id: string };
    const credentialId = Number(id);
    ...
    await prisma.$transaction(async (tx) => {
      await tx.googleCredential.update({
        where: { id: credentialId },
    ...
    await redis.lrem('GLOBAL_CREDENTIAL_POOL', 0, String(credentialId));
    await redis.lrem('GLOBAL_CREDENTIAL_POOL_V3', 0, String(credentialId));
    await redis.srem('COOLING_SET', String(credentialId));

    return { success: true, message: '凭证已禁用' };
  });
