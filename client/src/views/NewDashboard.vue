<template>
  ...
  >
        ...
        <div class="ag-card-style rounded-[40px] p-8 text-white">
           ...
           <h3 class="text-xl font-bold mb-6 text-[#C4B5FD]">我的上传记录</h3>
             <div class="flex items-center gap-3 flex-wrap">
               <div v-if="selectedCredentialIds.length > 0" class="flex items-center gap-2 text-[10px] text-white/70 bg-white/5 px-3 py-1 rounded-full border border-white/10">
                 <span>已选 {{ selectedCredentialIds.length }}</span>
                 <button
                   @click="batchDisableCredentials"
                   class="px-2 py-1 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/20 transition-colors"
                 >
                   批量禁用
                 </button>
                 <button
                   @click="batchDeleteCredentials"
                   class="px-2 py-1 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/20 transition-colors"
                 >
                   批量删除
                 </button>
               </div>
               <div v-if="myCredentials.length > 0" class="flex items-center gap-2 text-xs text-white/60">
                 <span>{{ credentialsPage }} / {{ credentialsPageCount }}</span>
                 <button
                   @click="prevCredentialsPage"
                   :disabled="credentialsPage <= 1"
                   class="px-2 py-1 bg-white/10 hover:bg-white/20 disabled:opacity-50 rounded-lg text-white text-xs font-bold"
                 >
                   <
                 </button>
                 <button
                   @click="nextCredentialsPage"
                   :disabled="credentialsPage >= credentialsPageCount"
                   class="px-2 py-1 bg-white/10 hover:bg-white/20 disabled:opacity-50 rounded-lg text-white text-xs font-bold"
                 >
                   >
                 </button>
           ...
           <div v-else class="space-y-3">
             <div class="flex items-center justify-between px-1 text-xs text-white/50">
               <label class="flex items-center gap-2">
                 <input
                   type="checkbox"
                   :checked="allVisibleSelected"
                   @change="toggleSelectAllVisible"
                   class="h-4 w-4 rounded border-white/20 bg-black/20 text-[#8b5cf6] focus:ring-[#8b5cf6]"
                 />
                 <span>全选本页</span>
               </label>
             </div>
             <div v-for="(cred, index) in visibleCredentials" :key="cred.id"
                  @click="toggleCredentialSelection(cred.id)"
                  class="flex items-center justify-between p-4 rounded-2xl transition bg-white/5 hover:bg-white/10 cursor-pointer"
                  :class="{ 'bg-white/10 ring-1 ring-purple-400/30': selectedCredentialIds.includes(cred.id) }"
             >
                <div class="flex items-center gap-4">
                  ...
                  <div class="w-10 h-10 rounded-full flex items-center justify-center text-xl bg-white/10">
                    {{ credentialStatusIcon(cred.status) }}
                  </div>
                  <div class="flex-1 min-w-0">
                    <div class="font-bold flex items-center gap-2 flex-wrap">
                        <span class="break-all">{{ cred.google_email || `凭证 #${index + 1}` }}</span>
                        <span class="text-[10px] px-2 py-0.5 rounded whitespace-nowrap" :class="credentialStatusClass(cred.status)">
                          {{ credentialStatusLabel(cred.status) }}
                        </span>
                ...
                <div class="flex items-center gap-2">
                    <button @click.stop="deleteCredential(cred.id)" class="text-sm text-red-300 hover:text-red-100 px-3 py-1 rounded-lg hover:bg-red-500/20 transition">
            ...
            <div class="usage-guide" style="border-top: none; padding-top: 0;">
                ...
                <div class="usage-steps">
                    ...
                    <ol class="list-decimal list-inside space-y-1 text-sm">
                        ...
                        <li>模型: gemini-2.5-flash / gemini-2.5-pro / gemini-3-pro-preview / gemini-3-flash-preview</li>
                        <li>Gemini 3 支持 thinking_level（Pro: low/high；Flash: minimal/medium/low/high）</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <div v-else-if="currentTab === 'admin' && isAdmin" class="max-w-6xl mx-auto text-white">
        ...
        <div class="admin-monitoring-container mb-8">
            <AdminMonitoringBar />
        </div>
...
import AdminMonitoringBar from '../components/AdminMonitoringBar.vue';
...
const credentialsPerPage = ref(5);
...
const credentialsPageCount = computed(() => {
    return Math.max(1, Math.ceil(myCredentials.value.length / credentialsPerPage.value));
});

const visibleCredentials = computed(() => {
    const start = (credentialsPage.value - 1) * credentialsPerPage.value;
    return myCredentials.value.slice(start, start + credentialsPerPage.value);
});

const allVisibleSelected = computed(() => {
    if (visibleCredentials.value.length === 0) return false;
    return visibleCredentials.value.every(cred => selectedCredentialIds.value.includes(cred.id));
});
...
const syncCredentialsPage = () => {
    if (credentialsPage.value > credentialsPageCount.value) {
        credentialsPage.value = credentialsPageCount.value;
    }
};

const prevCredentialsPage = () => {
    if (credentialsPage.value > 1) {
        credentialsPage.value -= 1;
        selectedCredentialIds.value = [];
    }
};

const nextCredentialsPage = () => {
    if (credentialsPage.value < credentialsPageCount.value) {
        credentialsPage.value += 1;
        selectedCredentialIds.value = [];
    }
...
const toggleSelectAllVisible = () => {
    const visibleIds = visibleCredentials.value.map(cred => cred.id);
    if (allVisibleSelected.value) {
        selectedCredentialIds.value = selectedCredentialIds.value.filter(id => !visibleIds.includes(id));
    } else {
        const merged = new Set([...selectedCredentialIds.value, ...visibleIds]);
        selectedCredentialIds.value = Array.from(merged);
    }
};

const toggleCredentialSelection = (id: number) => {
    if (selectedCredentialIds.value.includes(id)) {
        selectedCredentialIds.value = selectedCredentialIds.value.filter(item => item !== id);
    } else {
        selectedCredentialIds.value = [...selectedCredentialIds.value, id];
    }
};

const credentialStatusLabel = (status: string) => {
    if (status === 'COOLING') return '冷却中';
    if (status === 'DEAD') return '已失效';
    return '使用中';
};

const credentialStatusClass = (status: string) => {
    if (status === 'COOLING') return 'bg-amber-500/20 text-amber-300';
    if (status === 'DEAD') return 'bg-red-500/20 text-red-300';
    return 'bg-emerald-500/20 text-emerald-300';
};

const credentialStatusIcon = (status: string) => {
...
const deleteCredential = async (id: number, confirmDelete: boolean = true, refresh: boolean = true) => {
    if (confirmDelete && !confirm('确定删除此凭证？')) return;
    try {
...
const batchDisableCredentials = async () => {
    if (selectedCredentialIds.value.length === 0) return;
    if (!confirm(`确认批量禁用 ${selectedCredentialIds.value.length} 个凭证？`)) return;
    const failed: number[] = [];
    for (const id of selectedCredentialIds.value) {
        try {
            await api.post(`/credentials/${id}/disable`);
        } catch {
            failed.push(id);
        }
    }
    ...
    fetchStats();
    if (failed.length > 0) {
        alert(`部分禁用失败: ${failed.join(', ')}`);
    }
};

const batchDeleteCredentials = async () => {
    if (selectedCredentialIds.value.length === 0) return;
    if (!confirm(`确认批量删除 ${selectedCredentialIds.value.length} 个凭证？此操作不可恢复。`)) return;
    const failed: number[] = [];
    for (const id of selectedCredentialIds.value) {
        try {
            await api.delete(`/credentials/${id}`);
...
const toggleKey = async (key: any) => {
    ...
    const nextState = !key.is_active;

    if (target) target.is_active = nextState;

    try {
        const res = await api.patch(`/dashboard/api-keys/${key.id}`, { is_active: nextState });
        const idx = apiKeys.value.findIndex(k => k.id === key.id);
        if (idx !== -1) {
            apiKeys.value[idx] = { ...apiKeys.value[idx], ...res.data };
        } else {
            await refreshApiKeys();
        }
    } catch (e: any) {
