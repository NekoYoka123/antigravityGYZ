  ...
  <div class="flex flex-col h-full space-y-4">
    ...
    <div class="flex items-center justify-between bg-white/5 backdrop-blur-xl p-3 rounded-2xl border border-purple-500/30 shadow-[0_0_15px_rgba(139,92,246,0.1)]">
      <div class="flex items-center gap-4 flex-wrap">
        <div class="relative">
          <input
            ...
            placeholder="🔍 ID / 邮箱 / Discord / 用户ID"
            class="w-56 bg-black/20 border border-white/10 rounded-xl px-4 py-2 pl-9 text-sm text-white focus:outline-none focus:border-[#8B5CF6] transition-all placeholder-[#A5B4FC]/60"
          />
          <span class="absolute left-3 top-2 text-xs text-[#A5B4FC]">🔎</span>
        </div>
        <!-- Filter Dropdown -->
      ...
      <div class="flex items-center gap-3">
        <div v-if="selectedIds.length > 0" class="flex items-center gap-2 text-[10px] font-bold text-[#A5B4FC] bg-white/5 px-3 py-1 rounded-full border border-white/10">
          ...
          <button
            @click="batchDelete"
            class="px-2 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/20 transition-colors"
          >
            批量删除
          </button>
        </div>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="text-left bg-white/5 border-b border-white/10">
            <th class="px-6 py-4 text-[10px] font-black text-[#A5B4FC] uppercase tracking-widest">
              <input
                type="checkbox"
        ...
        <tbody class="divide-y divide-white/5">
          ...
          <tr v-for="cred in credentials" :key="cred.id"
              @click="toggleRowSelection(cred.id)"
              :class="[
                'group hover:bg-white/5 transition-colors hover:shadow-[0_0_15px_rgba(139,92,246,0.05)]',
                selectedIds.includes(cred.id) ? 'bg-white/10' : ''
              ]">
            ...
            <td class="px-6 py-3 text-right">
              <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                ...
                <button
                  ...
                  @click.stop="toggleCredential(cred.id, true)"
                ...
                <button
                  v-else
                  @click.stop="toggleCredential(cred.id, false)"
...
const isAllSelected = computed(() => {
  return credentials.value.length > 0 && credentials.value.every(cred => selectedIds.value.includes(cred.id));
});
...
const toggleSelectAll = () => {
  if (isAllSelected.value) {
    ...
    selectedIds.value = credentials.value.map(cred => cred.id);
  }
};
...
const toggleCredential = async (id: number, enable: boolean, refresh: boolean = true) => {
  try {
    await api.post(`/admin/credentials/${id}/toggle`, { enable });
    if (refresh) await fetchCredentials();
  } catch (err) {
    alert('状态修改失败');
  }
};
...
const deleteCredential = async (id: number, confirmDelete: boolean = true, refresh: boolean = true) => {
  if (confirmDelete && !confirm(`确认删除 #${id}？这会彻底移除该凭证。`)) return;
  try {
...
const batchDisable = async () => {
  ...
  if (!confirm(`确认批量禁用 ${selectedIds.value.length} 个凭证？`)) return;
  const failed: number[] = [];
  for (const id of selectedIds.value) {
    try {
      await api.post(`/admin/credentials/${id}/toggle`, { enable: false });
    } catch {
      failed.push(id);
    }
  }
...
const batchDelete = async () => {
  if (selectedIds.value.length === 0) return;
  if (!confirm(`确认批量删除 ${selectedIds.value.length} 个凭证？此操作不可恢复。`)) return;
  const failed: number[] = [];
  for (const id of selectedIds.value) {
    try {
      await api.delete(`/admin/credentials/${id}`);
